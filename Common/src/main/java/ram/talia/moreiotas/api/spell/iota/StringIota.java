...
public class StringIota extends Iota {
    public StringIota(@NotNull String string) throws MishapInvalidIota {
        ...
        if (string.length() > MoreIotasConfig.getServer().getMAX_STRING_LENGTH())
            throw MishapInvalidIota.of(this, 0, "string.max_size", MoreIotasConfig.getServer().getMAX_STRING_LENGTH(), string.length());
    }

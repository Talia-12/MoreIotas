...
public class ItemTypeIota extends Iota {
    public ItemTypeIota(@NotNull Item item) {
        super(ITEM_TYPE, Either.left(item));
    }
    public ItemTypeIota(@NotNull Block block) {
        super(ITEM_TYPE, Either.right(block));
    }
    ...
    public @NotNull Tag serialize() {
        ...
        return this.getEither().map(item -> {
            tag.putString(TAG_ITEM, BuiltInRegistries.ITEM.getKey(item).toString());
            return tag;
        }, block -> {
            tag.putString(TAG_BLOCK, BuiltInRegistries.BLOCK.getKey(block).toString());
            return tag;
        });
    }
    ...
    public static IotaType<ItemTypeIota> TYPE = new IotaType<>() {
        ...
        public ItemTypeIota deserialize(Tag tag, ServerLevel world) throws IllegalArgumentException {
            ...
            if (ctag.contains(TAG_ITEM)) {
                ...
                var type = typeLocation.map(BuiltInRegistries.ITEM::get).get().left();
                ...
                var type = typeLocation.map(BuiltInRegistries.BLOCK::get).get().left();
